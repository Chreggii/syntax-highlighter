...
export class FileHighlighterController {
    ...
    async highlightText(@Query() query: { sourceText: string, language: string }): Promise<any> {
        ...
        if (languages.includes(query.language)) {
            ...
            const mlClassifierData = (await this.httpService.get(`http://mlclassifier:3000/ml-highlight?lexing=${JSON.stringify(lexData)}&language=${query.language}`).toPromise()).data;
